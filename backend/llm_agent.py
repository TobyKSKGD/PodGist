...
# 模型降级梯队：平衡质量和速度，qwen-plus 优先（质量够用、速度合理）
MODEL_FALLBACK_LIST = ['qwen-plus', 'qwen-turbo']
...
def get_podcast_summary_robust(api_key, podcast_text, max_timeline_items=15):
    ...
    # ---------- 构建基础 prompt ----------
    # 格式必须完全精确，每行开头固定，不允许模型随意套格式
    prompt_base = f"""
请阅读以下【带有时间戳】的音频逐字稿，输出一期播客的结构化摘要。

【格式要求】严格按以下每一行的格式输出，不允许改变结构，不允许添加说明文字：

第1行：纯文本标题（不要任何前缀符号、不要换行，直接写标题）
第2行：空行
第3行：以>开头写总结引擎，例如：> 总结引擎：通义千问
第4行：以>开头写核心关键词，例如：> 核心关键词：关键词1，关键词2
第5行：空行
第6行：以### 节目总体概述开头（作为一级小标题）
第7行起：正文段落（至少200字，可分段）
空行
空行
以### 细致高光时间轴开头（作为一级小标题）
接下来每行一个时间轴条目，格式为：- [MM:SS] 事件描述（不要省略任何括号和横线）
